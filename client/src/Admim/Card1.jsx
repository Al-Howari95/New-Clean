...
const Card1 = () => {
  ...
  const handleAddCard = () => {
    if (isEditing) {
      axios
        .put(`http://localhost:4000/MyData/${newCard.id}`, newCard)
        .then(() => {
          setIsEditing(false);
      ...
      axios
        .post("http://localhost:4000/MyData", newCard)
        .then(() => {
          setNewCard({ title: "", description: "", image: "" });
          fetchData();
        })
  ...
  const handleDeleteCard = (id) => {
    axios
      .delete(`http://localhost:4000/MyData/${id}`)
      .then(() => {
        fetchData();
      })
                ...
                <div className="p-4">
                  ...
                  <div className="flex justify-between">
                    <button
                      ...
                      onClick={() => handleEditCard(key)}
                    >
                      Edit
                    </button>
                    <button
                      className="text-red-500 hover:underline"
                      onClick={() => handleDeleteCard(key.id)}
                    >
                      Delete
                    </button>
                  </div>
